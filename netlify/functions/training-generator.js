...
exports.handler = async function (event) {
  ...
  try {
    ...
    var lengthSetting = body.length || "standard";
    ...
    /* ── Length targets based on dropdown selection ── */
    var lengthMap = {
      brief:         { words: 500,  min: 400,  max: 650,   tokens: 4000  },
      standard:      { words: 1000, min: 800,  max: 1200,  tokens: 5000  },
      detailed:      { words: 2000, min: 1700, max: 2400,  tokens: 8000  },
      comprehensive: { words: 3500, min: 3000, max: 4000,  tokens: 12000 },
      exhaustive:    { words: 5000, min: 4500, max: 6000,  tokens: 16000 }
    };
    var len = lengthMap[lengthSetting] || lengthMap.standard;
    var targetWords = len.words;
    var minWords = len.min;
    var maxWords = len.max;
    var dynamicMaxTokens = len.tokens;
    ...
    userPrompt += "REQUIRED LENGTH: " + targetWords + " words (between " + minWords + " and " + maxWords + " words). This is a HARD REQUIREMENT, not a suggestion. You must produce a document within this word range. If you are under " + minWords + " words, continue adding substantive content, examples, and detail until you reach the target.\n";
